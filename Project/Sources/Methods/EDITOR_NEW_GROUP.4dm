...
End if 
  ...
  // ----------------------------------------------------
If (Asserted:C1132(xml_IsValidReference (String:C10(Form:C1466.file.$dom))))
	
	  // Create the group [
	$Txt_resname:=Get localized string:C991("NewGroup")
	$Lon_count:=Form:C1466.file.groups.length
	$Txt_resname:=Choose:C955($Lon_count=0;$Txt_resname;$Txt_resname+" "+String:C10($Lon_count+1))
	
	$Dom_group:=DOM Create XML element:C865(Form:C1466.file.$dom;"/xliff/file/body/group";\
		"resname";$Txt_resname)
	  //]
	...
	If (Asserted:C1132(OK=1))
		...
		For each ($Obj_language;Form:C1466.languages)
			...
			If (Asserted:C1132(OK=1))
				  ...
				  // Set & save the file
				Form:C1466.save($Dom_node;$Obj_language.file.nativePath)
		...
		If (Form:C1466.file.groups=Null:C1517)
			
			Form:C1466.file.groups:=New collection:C1472
			
		End if 
		
		Form:C1466.file.groups.push(Form:C1466.$current)
