...
Case of 
	...
	: ($Obj_in.target="localization")
		...
		$File_xlf:=Form:C1466.languages[Form:C1466.languages.extract("language").indexOf($Obj_in.language)].file.nativePath
	...
	: ($Obj_in.target="source")
		...
		For each ($Obj_language; Form:C1466.languages)
			...
			// Set & save the file
			Form:C1466.setValue($Dom_source; Form:C1466.$current.source.value; $Obj_language.file.nativePath)
	...
	: ($Obj_in.target="translate")
		...
		For each ($Obj_language; Form:C1466.languages)
			...
			// Set & save the file
			Form:C1466.setAttribute($Dom_unit; $Obj_in.target; ->$Txt_value; $Obj_language.file.nativePath)
	...
	: ($Obj_in.target="note")
		...
		If (Length:C16($Txt_value)#0)
			...
			For each ($Obj_language; Form:C1466.languages)
				...
				// Set & save the file
				Form:C1466.setValue($Dom_note; $Txt_value; $Obj_language.file.nativePath)
		...
		Else 
			...
			For each ($Obj_language; Form:C1466.languages)
				...
				If (OK=1)
					...
					// Set & save the file
					xlf_DELETE_ELEMENT($Dom_note; $Obj_language.file.nativePath)
	...
	: ($Obj_in.target="propagate_reference")
		...
		For each ($Obj_language; Form:C1466.languages)
			...
			If ($Obj_language.language#Form:C1466.language)  // Not for the reference file
				...
				Form:C1466.setValue(Form:C1466.$current[$Obj_language.language].$dom; Form:C1466.$current.source.value; $Obj_language.file.nativePath)
	...
	: ($Obj_in.target="id")\
		...
		For each ($Obj_language; Form:C1466.languages)
			...
			ASSERT:C1129(xml_IsValidReference($Dom_target))
			
			// Set & save the file
			DOM SET XML ATTRIBUTE:C866($Dom_target; \
				$Obj_in.target; $Obj_in.value)
			
			Form:C1466.save($Dom_target; $Obj_language.file.nativePath)
	...
	: ($Obj_in.target="duplicateResname")
		...
		Form:C1466.file.duplicateResname:=EDITOR_Highlight_duplicated(Form:C1466.dynamic("unit"); Form:C1466.dynamic("content"))
