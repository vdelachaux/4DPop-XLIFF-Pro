...
If (OK=1)
	...
	If (Form:C1466.$target="unit")
		...
		For each ($Obj_language; Form:C1466.languages)
			...
			Else 
				...
				If (Bool:C1537(Form:C1466.$uniqueID))
			...
			If (xml_IsValidReference($Dom_unit))
				...
				// Save the file
				Form:C1466.save($Dom_; $Obj_language.file.platformPath)
	...
	Else 
		...
		// Update project [
		$Lon_x:=Form:C1466.$groups.indexOf(Form:C1466.$current)
		$Lon_number:=Form:C1466.$groups[$Lon_x].units.length  // The number of lines to remove
		
		// Remove the group
		Form:C1466.$groups.remove($Lon_x; 1)
		//]
