...
If (Asserted:C1132($Lon_parameters>=1; "Missing parameter"))
	...
	Form:C1466.$currentFile:=(obj_Dynamic("file"))->{$Lon_row}
	...
	If (Form:C1466.file.platformPath#Null:C1517)
		
		Form:C1466.$dom:=_o_xlf_Open(Form:C1466.file.platformPath)
		
	End if 
	
	ASSERT:C1129(xml_IsValidReference(Form:C1466.$dom))
	//}
...
// ----------------------------------------------------
ASSERT:C1129(Not:C34(Bool:C1537(Form:C1466.$shared)))  //#IN_WORKS - must manage files at the root of the resources folder

$Dom_file:=DOM Find XML element:C864(Form:C1466.$dom; "/xliff/file")
...
If (Asserted:C1132(Bool:C1537(OK)))
	...
	Form:C1466.$datatype:=String:C10(xml_attributes($Dom_file).datatype)
	
	//End if 
	
	$tDom_group{0}:=DOM Find XML element:C864($Dom_file; "/xliff/file/body/group"; $tDom_group)
	...
	If (Bool:C1537(OK))
		...
		For ($Lon_group; 1; Size of array:C274($tDom_group); 1)
			...
			$tDom_unit{0}:=DOM Find XML element:C864($tDom_group{$Lon_group}; "trans-unit"; $tDom_unit)
			...
			If ($Lon_unitCount>0)
				...
				For ($Lon_unit; 1; $Lon_unitCount; 1)
					...
					$Dom_source:=DOM Find XML element:C864($tDom_unit{$Lon_unit}; "source")
					...
					If (OK=1)
						...
						$Dom_source:=DOM Find XML element:C864($tDom_unit{$Lon_unit}; "note")
			...
			Form:C1466.$groups.push($Obj_group)
		...
		Form:C1466.$duplicateResname:=Not:C34($Col_allUnits.extract("resname").orderBy().equal($Col_allUnits.distinct("resname"; ck diacritical:K85:3); ck diacritical:K85:3))
		
		If (Form:C1466.$duplicateResname)
		...
		Form:C1466.$uniqueID:=$Col_allUnits.extract("id").orderBy().equal($Col_allUnits.distinct("id"; ck diacritical:K85:3); ck diacritical:K85:3)
		
	Else 
		
		Form:C1466.$uniqueID:=True:C214
		Form:C1466.$duplicateResname:=False:C215
		
	End if 
	
	// [Localizations] {
	If (Bool:C1537(Form:C1466.$shared))  //#IN_WORKS - must manage files at the root of the resources folder
	...
	Else 
		...
		For each ($Obj_language; Form:C1466.languages)
			...
			If ($Obj_language.language=Form:C1466.language)
				...
				$Obj_language.file.platformPath:=$Obj_language.file.parentPath+$Obj_language.file.fullName
			...
			Else 
				...
				If (Asserted:C1132(Test path name:C476($Obj_language.file.platformPath)=Is a document:K24:1))
					
					// Open the file
					If ($Obj_language.file.$dom=Null:C1517)
						
						$Dom_root:=_o_xlf_Open($Obj_language.file.platformPath)
