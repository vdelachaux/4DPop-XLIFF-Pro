...
C_TEXT:C284($kTxt_document;$kTxt_folder;$kTxt_Separator;$Mnu_popup;$Txt_;$Txt_action)
...
$kTxt_folder:=convert_Unicode_to_UTF16 ("1F4C1")
$kTxt_document:=convert_Unicode_to_UTF16 ("1F4C4")
...
Case of 
	...
	: ($Lon_formEvent=On Clicked:K2:4)
		...
		Case of 
				  ...
				  //……………………………………………………………………………………………………………………
			: (Application type:C494=4D Remote mode:K5:5) & ($Lon_platform=Mac OS:K25:2) & (Position:C15("\\";$Txt_path)>0)
				
				$kTxt_Separator:="\\"
				
				  //……………………………………………………………………………………………………………………
			: (Application type:C494=4D Remote mode:K5:5) & ($Lon_platform=Windows:K25:3) & (Position:C15(":";Replace string:C233($Txt_path;":";"";1))>0)
			...
			If ($Txt_path[[$Lon_i]]=$kTxt_Separator)
				...
				Case of 
					...
					: (Find in array:C230($tTxt_volumes;$Txt_)>0)
						...
						$Txt_buffer:=convert_Unicode_to_UTF16 ("1F4BE")+" "+$Txt_buffer
						
						  //-------------------------------------------
					: (Test path name:C476($Txt_)=Is a folder:K24:2)
						
						$Txt_buffer:=$kTxt_folder+" "+$Txt_buffer
						
						  //-------------------------------------------
					: (Test path name:C476($Txt_)=Is a document:K24:1)
						
						$Txt_buffer:=$kTxt_document+" "+$Txt_buffer
		...
		If (Length:C16($Txt_Buffer)>0)
			...
			$Txt_buffer:=Choose:C955(Test path name:C476($Txt_path)=Is a document:K24:1;$kTxt_document;$kTxt_folder)+" "+$Txt_buffer
		...
		If (Count menu items:C405($Mnu_popup)>0)
			...
			If (OB Get:C1224($Obj_widget;"showOnDisk";Is boolean:K8:9)) | (OB Get:C1224($Obj_widget;"copyPath";Is boolean:K8:9))
